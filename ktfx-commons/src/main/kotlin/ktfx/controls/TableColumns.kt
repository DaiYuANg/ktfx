...
@DslMarker @Target(AnnotationTarget.CLASS) annotation class TableColumnDslMarker
...
fun <S> TableView<S>.columns(configuration: TableColumnScope<S>.() -> Unit) {
  ...
  object : TableColumnScope<S> {
        override val columns: MutableCollection<TableColumn<S, *>>
          get() = columns2
      }
      .configuration()
}
...
fun <S> TreeTableView<S>.columns(configuration: TreeTableColumnScope<S>.() -> Unit) {
  ...
  object : TreeTableColumnScope<S> {
        override val columns: MutableCollection<TreeTableColumn<S, *>>
          get() = columns2
      }
      .configuration()
}
