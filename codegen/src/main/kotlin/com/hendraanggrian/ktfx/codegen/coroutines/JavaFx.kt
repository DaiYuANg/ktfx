...
private fun ListenerFactory.initJavaFx() {
  ...
  (TableView::class.name.parameterizedBy(S)) {
    "setOnSort" {
      action(
          SortEvent::class.name.parameterizedBy(TableView::class.name.parameterizedBy(S)),
      )
    }
    "setOnScrollTo" { action(ScrollToEvent::class.parameterizedBy(Int::class)) }
    "setOnScrollToColumn" {
  ...
  (TreeTableView::class.name.parameterizedBy(T)) {
    "setOnSort" {
      action(
          SortEvent::class.name.parameterizedBy(TreeTableView::class.name.parameterizedBy(T)),
      )
    }
    "setOnScrollTo" { action(ScrollToEvent::class.parameterizedBy(Int::class)) }
    "setOnScrollToColumn" {
