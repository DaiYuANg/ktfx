...
data class LayoutsEntry(
  ...
  val customTypeName: TypeName
    get() =
        typeName.takeUnless { customClass }
            ?: classNamed(
                packageName,
                when {
        ...
        simpleName
            .mapIndexed { index, c ->
              when {
                ...
                c.isUpperCase() && simpleName.getOrNull(index + 1)?.isUpperCase() ?: false ->
                    c.lowercaseChar()
                else -> c
              }
            }
  ...
  val styledFunctionName: String
    get() = "styled$simpleName"

  val supportStyledFunction: Boolean
    get() = classNamed("ktfx.layouts", "PathElementContainer") !in containerClassNames
