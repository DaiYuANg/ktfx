...
object LayoutsWriter {
            ...
            functions {
              entry.containerClassNames.forEach { containerClassName ->
                entry.functionName {
                  ...
                  parameters { entry.parameters.forEach(::parameter) }
                  appendLine(
  ...
  private fun ParameterSpecHandlerScope.styleClass() =
      parameter("styleClass", String::class, VARARG) { kdoc("the CSS style class.") }
  ...
  private fun ParameterSpecHandlerScope.configuration(entry: LayoutsEntry) =
      parameter(
          "configuration",
          entry.customTypeName.annotate(DSL_MARKER.asAnnotationSpec()).lambdaBy(returns = UNIT),
      ) {
