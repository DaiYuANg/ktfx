...
open class LayoutsFactory(
  ...
  companion object {
    ...
    fun ParameterSpecHandlerScope.color() =
        "color"(Color::class.name) { defaultValue("%M", Color::class.name.member("WHITE")) }
    ...
    fun ParameterSpecHandlerScope.treeItem(name: String, variable: TypeVariableName) =
        name(TreeItem::class.name.parameterizedBy(variable).copy(true)) { defaultValue("null") }
