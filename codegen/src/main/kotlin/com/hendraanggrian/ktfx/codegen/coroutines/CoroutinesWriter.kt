              ...
              functions {
                functionEntry.simpleFunctionName {
                  ...
                  if (classEntry.receiver is ParameterizedTypeName) {
                    classEntry.receiver.typeArguments
                        .filter { it != STAR }
                        .forEach { typeVariable(it.toString().generics) }
                  }
                  ...
                  parameters { functionEntry.parameters.forEach(::parameter) }
                  ...
                  appendLine(
                      ...
                      when ("coroutines") {
                        !in factory.path ->
                            ...
                            buildString {
                              val hasParam =
                                  (parameters.last().type as LambdaTypeName).parameters.isNotEmpty()
                              append("${functionEntry.functionName} {")
                              if (hasParam) {
                                append(" event ->")
                              }
                              append(
                                  " GlobalScope.launch(context) { " + "${parameters.last().name}(",
                              )
                              if (hasParam) {
                                append("event")
                              }
